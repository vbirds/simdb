...
public:
    static void        FreeAnon(SharedMem& sm, bool bNeedRm = false)
    {
#ifdef _WIN32
        if(sm.hndlPtr){
...
public:
    ...
    bool       close(bool bNeedRm = false)
    {
        if(m_isOpen){
            m_isOpen = false;
            i32 prev = s_flags->fetch_sub(1);                                                   // prev is previous flags value - the number of simdb instances across process that had the shared memory file open
            if(prev <= 1) {                                                                        // if the previous value was 1, that means the value is now 0, and we are the last one to stop using the file, which also means we need to be the one to clean it up
                SharedMem::FreeAnon(m_mem, bNeedRm);                                                       // close and delete the shared memory - this is done automatically on windows when all processes are no longer accessing a shared memory file
                return true;
            }
        }
        return false;
    ...
    static auto simdb_listDBs(simdb_error* error_code=nullptr) -> std::vector<std::string>
    {
        using namespace std;
